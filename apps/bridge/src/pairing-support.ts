...
export class BridgePairingStore {
  constructor(private readonly db: { prepare: (sql: string) => any; transaction: <T>(fn: () => T) => () => T }) {}

  createPairingSession(hostPublicKey: string): PairingSessionRecord {
  ...
  completePairing(devicePublicKey: string, deviceName: string, nonce: string): DeviceGrantRecord {
    return this.db.transaction(() => {
      ...
      const pairedAt = existing?.paired_at ?? now;
      ...
      if (existing) {
        this.db
          ...
          .run(deviceName, grantJson, now, deviceId);
      } else {
        this.db
          .prepare(
            `INSERT INTO devices (
          ...
          .run(deviceId, deviceName, devicePublicKey, grantJson, pairedAt, now);
      }

      this.db
        .prepare(`UPDATE pairing_sessions SET used_at = ? WHERE nonce = ?`)
        .run(now, nonce);
      this.insertAuditEvent('pairing.completed', deviceId, { deviceName });
  ...
  validateDevice(devicePublicKey: string): DeviceGrantRecord | null {
    const row = this.db
      .prepare(
  ...
  createQrPayload(hostId: string, hostName: string, hostPublicKey: string, endpoints: string[]): BridgeQrPayload {
    const session = this.createPairingSession(hostPublicKey);
    return {
      v: 1,
      hostId,
      hostName,
  ...
  private insertAuditEvent(type: string, deviceId: string | null, metadata: Record<string, unknown>): void {
    this.db
      .prepare(
        `INSERT INTO audit_events (id, device_id, session_id, type, metadata_json, created_at)
         VALUES (?, ?, NULL, ?, ?, ?)`
      )
      .run(randomUUID(), deviceId, type, JSON.stringify(metadata), new Date().toISOString());
  }
}
